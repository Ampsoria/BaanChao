...
```
Rent, utility, and deposit management for a small rental property with flexible room inventory.
...
ระบบจัดการค่าเช่าบ้านเช่าที่มีห้องเริ่มต้น 6 ห้อง และเพิ่ม/ปิดใช้งานห้องได้ ครอบคลุม:
...
| ไทย | โค้ด | หมายเหตุ |
|-----|------|----------|
| ห้องพัก | `Room` | seed เริ่มต้น 6 ห้อง; เพิ่มได้และปิดใช้งานแบบรักษาประวัติ |
...
ตารางนี้เป็นค่าเริ่มต้นของห้อง 1–6 เท่านั้น ห้องใหม่เพิ่มจากหน้า Admin ได้โดยกำหนดเลขห้องและค่าเช่า
ระบบจัดเศษสตางค์ว่าง `.01–.99` ให้อัตโนมัติ จึงรองรับได้สูงสุด 99 ห้องต่ออาคาร ห้องที่เลิกใช้ให้
ปิดใช้งานแทนการลบจริงเพื่อรักษาประวัติผู้เช่า มิเตอร์ บิล และการชำระเงิน และปิดไม่ได้ขณะยังมีผู้เช่า active
...
หน้าเดียวกันมีรายการห้องที่เปิดใช้งานทั้งหมด แต่ละห้องแสดงสถานะ แล้วมีปุ่มตามสถานะ
...
- ผู้ดูแลคนเดียว → cookie auth + user เดียวก็พอ ไม่ต้อง Identity เต็มรูปแบบ
...
**MonsterASP.NET** — รองรับสแตกที่ใช้ แต่ต้องยืนยันแพ็กเกจที่มี HTTPS ก่อนขึ้นจริง

- MSSQL รวมอยู่ในแพ็กเกจ ไม่ต้องจ่ายค่า database แยก
- .NET 10 และ MSSQL รองรับบนผู้ให้บริการนี้
- HTTPS เป็นข้อบังคับของระบบ แต่สิทธิ์ใน Free tier อาจเปลี่ยนตามราคา/นโยบายผู้ให้บริการ
...
| รายการ | ค่าใช้จ่าย |
|--------|-----------|
| Hosting + MSSQL + HTTPS + subdomain | ตรวจราคาปัจจุบัน — ใช้ Free ได้เฉพาะเมื่อมี HTTPS |
| LINE OA แพ็กเกจ Free | ฟรี |
| QR พร้อมเพย์บุคคล | ฟรี |
| Backup ขึ้น Google Drive | ฟรี (15GB) |
| Domain | ไม่ซื้อ |
| **รวม** | **0 บาทเมื่อแพ็กเกจฟรีมี HTTPS; ไม่เช่นนั้นต้องมีค่าโฮสต์** |
...
**Phase 1 — แกนหลัก**
- [x] สร้าง schema + seed ห้องเริ่มต้น 6 ห้อง พร้อมเพิ่ม/ปิดใช้งานห้องภายหลัง
...
- **ไม่รองรับหลายอาคาร** แต่เพิ่มหรือลดห้องภายในอาคารเดิมได้ ห้องที่ลดจะปิดใช้งานเพื่อเก็บประวัติ
...
- **ไม่เชื่อม API ธนาคารโดยตรง** ต้องเป็น merchant ถึงทำได้ ไม่คุ้มกับบ้านเช่าขนาดเล็ก
...
| ข้อมูลผู้เช่าปัจจุบันทุกห้อง (ชื่อ วันเข้าอยู่ มัดจำที่ถืออยู่) | **รอ Amp ให้ข้อมูล — บล็อกการเริ่มใช้จริง** |
