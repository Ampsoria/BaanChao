...
> เอกสารนี้เป็น **แผนโปรเจกต์** ยังไม่ได้เริ่มเขียนโค้ด
> ใช้เป็น context ให้ Claude Code เวลากลับมาทำงานต่อ
...
| 10 | EF Core + Config | map entity / ตั้งค่า |
| 11 | Validation และ Error | เขียน validation |
| 12 | Testing | เขียนเทส |
| 13 | Roadmap | เลือกงานถัดไป |
| 14 | สิ่งที่ระบบนี้จะไม่ทำ | ก่อนเพิ่มฟีเจอร์ใหม่ |
| 15 | เรื่องที่ยังต้องตัดสินใจ | เจอจุดที่ไม่มีคำตอบในไฟล์นี้ |
...
└── BaanChao.Tests/              # เคสตามหัวข้อ 12
```
...
- [ ] เดือนเต็ม 30 วัน คิดค่าเช่าเต็ม
- [ ] เข้าอยู่วันที่ 17 เดือน 30 วัน → `FLOOR(2000 × 14 / 30)` = 933
- [ ] เข้าอยู่วันที่ 1 → ไม่ prorate
- [ ] เข้าอยู่วันสุดท้ายของเดือน → คิด 1 วัน
- [ ] เดือน ก.พ. 28 วัน กับปีอธิกสุรทิน 29 วัน
- [ ] เดือนที่ไม่เต็ม ไม่คิดค่าขยะ
- [ ] อัตราเปลี่ยนกลางทาง → บิลเก่าไม่เปลี่ยนตาม
- [ ] `UtilityPeriod` เป็นเดือนก่อน `BillingPeriod` เสมอ
- [ ] สั่งออกบิลซ้ำงวดเดิม → ไม่เกิดบิลใบที่สอง
- [ ] ห้องเปลี่ยนผู้เช่ากลางเดือน → ออกบิล 2 ใบในงวดเดียว

### มัดจำและการย้ายออก

- [ ] อยู่ 8 เดือน หัก 380 → คืน 1,620
- [ ] อยู่ 3 เดือน หัก 380 → ริบ 1,620 คืน 0
- [ ] อยู่ 4.53 เดือน → **ไม่ริบ** (ปัดขึ้นเป็น 5)
- [ ] อยู่ 4.33 เดือน → **ริบ**
- [ ] ค่าน้ำค่าไฟ 2,300 มัดจำ 2,000 → เก็บเพิ่ม 300 คืน 0
- [ ] ริบ + หักเกินมัดจำ → ยังเก็บส่วนเกินได้
- [ ] `RefundAmount` กับ `AmountDueFromTenant` ไม่เป็นบวกพร้อมกัน
- [ ] มีค่าเช่าค้าง → หักจากมัดจำ ไม่ยกให้
- [ ] ย้ายออกไม่คิดค่าเช่างวดสุดท้ายเพิ่ม

### สลิปและการชำระเงิน

- [ ] ส่งสลิปเดิมซ้ำ → reject
- [ ] จ่ายบางส่วน → สถานะเป็น "ชำระบางส่วน"
- [ ] บันทึกเงินสดโดยไม่มีสลิป → ผ่านได้
- [ ] ยอดโอนกับเศษสตางค์ระบุห้องได้ถูกห้อง
...
**Phase 1 — แกนหลัก**
- [ ] สร้าง schema + seed ข้อมูล 6 ห้อง
- [ ] CRUD บันทึกเลขมิเตอร์
- [ ] `sp_GenerateMonthlyInvoices` + unit test
- [ ] เคส prorate: เข้ากลางเดือน (ออกกลางเดือนไม่ต้องเฉลี่ย)
- [ ] รับมัดจำตอนทำสัญญา + `sp_CreateMoveOutSettlement`
- [ ] ใบแจกแจงการหักมัดจำ (แสดงทุกรายการ + รูปหลักฐาน)
- [ ] หน้า admin ตั้งค่าราคา (ตามหัวข้อ 8) + auth
- [ ] หน้า admin ย้ายเข้า / ย้ายออก + `sp_CreateMoveInInvoice`
- [ ] หน้าเว็บ admin แบบง่าย (กรอกมิเตอร์สิ้นเดือน + ดูสถานะ)
- [ ] บันทึกการชำระเงินด้วยมือ + พิมพ์บิล/ใบเสร็จ PDF

**Phase 2 — LINE** (ชั้นเสริม ระบบต้องใช้งานได้ครบโดยไม่มี Phase นี้)
- [ ] ตั้ง LINE OA + webhook endpoint
- [ ] ผูก `LineUserId` กับห้อง (ส่งรหัสยืนยันครั้งแรก)
- [ ] Broadcast บิลรายเดือน + สร้าง QR พร้อมเพย์ระบุยอด
- [ ] รับรูปสลิป → เก็บลง storage + บันทึก Payment

**Phase 3 — อัตโนมัติ**
- [ ] `BackgroundService` ออกบิลวันที่ 1 อัตโนมัติ
- [ ] เตือนห้องค้างชำระวันที่ 5
- [ ] `LocalSlipVerifier` (ZXing + SlipHash กันซ้ำ)
- [ ] เปิดใช้ค่าปรับจ่ายช้าจากหน้า admin (`BillingPolicy`)
- [ ] ออกใบเสร็จ PDF

**Phase 4 — ถ้ามีงบ**
- [ ] เสียบ Slip Verification API ผ่าน `ISlipVerifier`

---

## 14. สิ่งที่ระบบนี้จะไม่ทำ
